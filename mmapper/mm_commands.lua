...
mm.alias_specs = {
  ...
  {"^mapper explore window%s*(.*)$", function(m) local ok, err = mm.explore_window.command(m[2]); if not ok then mm.warn(err) end end},
  {"^mapper explore%s*(.*)$", function(m) local ok, err = mm.explore.command(m[2]); if not ok then mm.warn(err) end end},
